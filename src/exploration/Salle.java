    ...
    */
    ...
    public void setAcces(Direction d)
    {
        ...
        //code
          ...
          // la direction est valide
    if (d != null) {
        
        Salle SalleVoisine = getVoisine(d);
        
        // la salle voisine si elle existe
        if (SalleVoisine != null) {
            ...
            // ajouter l inverse de la direction
            Direction inverseDirection = d.getInverse();
            if (!SalleVoisine.acces.contains(inverseDirection)) {
                SalleVoisine.acces.add(inverseDirection);
            }
        }
    }
    }
    ...
    {
        if(this.isVisible() && !(getPosition().equals(getPlateau().getJoueur().getPosition())))
        { return this.getSymbole();
        }else {
            if(getPosition().equals(getPlateau().getJoueur().getPosition())){
            return "J"+this.getSymbole().replaceFirst(" ", "");
            }else{
                return "   ";
            }
        }//"░░░"; // salle non visible
    ...
    public Salle(Position pos, Plateau p, Objet o)
    {
        ...
        initAcces(); // au départ, la salle n'est pas visible
    }
